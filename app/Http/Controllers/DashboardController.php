...
class DashboardController extends Controller
{
    ...
    /**
     * Modern Dashboard for Karyawan (Mobile-First Design)
     */
    public function modern()
    {
        ...
        if ($data['presensi']) {
            $statusHariIni = 'Hadir';
            $jamMasuk = Carbon::parse($data['presensi']->jam_in)->format('H:i');
        }
        ...
        $jamMulai = '08:00';
        $jamSelesai = '17:00';
        if ($data['karyawan'] && $data['karyawan']->kode_jam_kerja) {
            $jamKerjaData = DB::table('presensi_jamkerja')
                ->where('kode_jam_kerja', $data['karyawan']->kode_jam_kerja)
                ->first();
            if ($jamKerjaData) {
                $jamMulai = substr($jamKerjaData->jam_masuk, 0, 5);
                $jamSelesai = substr($jamKerjaData->jam_pulang, 0, 5);
                $jamKerja = $jamKerjaData->total_jam ?? '8';
            }
        }

        // Kehadiran Bulan Ini
        $kehadiranBulan = $data['rekappresensi'] ? $data['rekappresensi']->hadir : 0;
        $hariKerja = Carbon::now()->daysInMonth;
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $jadwalMinggu[] = [
                ...
                'status_class' => $date->isToday() ? 'active' : ($date->isPast() ? '' : 'upcoming'),
                'status_icon' => $date->isToday() ? 'circle-check' : ($date->isPast() ? 'clock' : 'clock'),
                'status_text' => $date->isToday() ? 'Hari Ini' : ($date->isPast() ? 'Selesai' : 'Mendatang'),
            ];
        }

        // Sisa Cuti
        $data['cutiTahunan'] = 12 - (DB::table('presensi_izincuti')
            ->where('nik', $userkaryawan->nik)
            ->whereYear('dari_tanggal', Carbon::now()->year)
            ->count() ?? 0);
        $data['cutiSakit'] = 5 - (DB::table('presensi_izinsakit')
            ->where('nik', $userkaryawan->nik)
            ->whereYear('dari_tanggal', Carbon::now()->year)
            ->count() ?? 0);
        $data['cutiBesar'] = 30;
        ...
        $data['statusHariIni'] = $statusHariIni;
        $data['kehadiranBulan'] = $kehadiranBulan;
        $data['hariKerja'] = $hariKerja;
        $data['jadwalMinggu'] = $jadwalMinggu;
        $data['cutiTahunan'] = $data['cutiTahunan'];
        $data['cutiSakit'] = $data['cutiSakit'];
        $data['cutiBesar'] = $data['cutiBesar'];

        return view('dashboard.karyawan-modern', $data);
    }
}
