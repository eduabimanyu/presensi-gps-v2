...
class PelanggaranController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'bunyi' => 'nullable|array',
            'dasar_uu' => 'nullable|array'
        ]);
        ...
        try {
            ...
            $pasal_pelanggaran = [];
            if ($request->has('pasal')) {
                foreach ($request->pasal as $index => $pasal_name) {
                    if (!empty($pasal_name)) {
                        $pasal_pelanggaran[] = [
                            'pasal' => $pasal_name,
                            'bunyi' => $request->bunyi[$index] ?? ''
                        ];
                    }
                }
            }
            ...
            $dasar_uu = [];
            if ($request->has('dasar_uu')) {
                foreach ($request->dasar_uu as $uu_item) {
                    if (!empty($uu_item)) {
                        $dasar_uu[] = $uu_item;
                    }
                }
    ...
    public function update(Request $request, $no_sp)
    {
        ...
        try {
            ...
            $pelanggaranRecord = Pelanggaran::where('no_sp', $no_sp)->firstOrFail();
            $pelanggaranRecord->update([
                'nik' => $request->nik,
                'tanggal' => $request->tanggal,
                'dari' => $request->dari,
                'sampai' => $request->sampai,
